...
export function newInquiryHtml({
  ...
  const metaRows = [
    budget ? `<tr><td style="padding:6px 0;border-bottom:1px solid #f0ede8;"><span style="font-size:12px;color:#999;display:inline-block;width:80px;">Budget</span><span style="font-size:13px;color:#1a1a1a;font-weight:600;">₦${Number(budget).toLocaleString()} / month</span></td></tr>` : '',
    timeline ? `<tr><td style="padding:6px 0;"><span style="font-size:12px;color:#999;display:inline-block;width:80px;">Timeline</span><span style="font-size:13px;color:#1a1a1a;font-weight:600;">${timeline}</span></td></tr>` : '',
  ].filter(Boolean).join('')
...
<head>
  ...
  <title>${newInquirySubject(founderName)}</title>
</head>
        ...
        <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
          ...
          <tr>
            <td style="background:#ffffff;border-radius:16px;padding:40px;border:1px solid #ebe9e4;">
              ...
              <p style="margin:0 0 6px;font-size:13px;color:#999;font-weight:500;text-transform:uppercase;letter-spacing:0.08em;">New Inquiry</p>
              <h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#1a1a1a;line-height:1.3;">
                ${founderName} wants to work with you
              </h1>
              <p style="margin:0 0 28px;font-size:14px;color:#777;">Hi ${creativeName}, you have a new inquiry waiting for your response.</p>

              <!-- Message preview -->
              <div style="background:#faf9f7;border-radius:10px;padding:20px;border-left:3px solid #6b1d2b;margin-bottom:24px;">
                <p style="margin:0;font-size:14px;color:#444;line-height:1.7;">${preview}</p>
              </div>

              ${metaRows ? `<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">${metaRows}</table>` : ''}
