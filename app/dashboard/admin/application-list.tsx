...
interface AdminApplicationsListProps {
  ...
  initialTotal: number
  initialFilter: ReviewStatus | 'all'
  initialPage: number
}
...
export default function AdminApplicationsList({
  ...
  const [filter, setFilter] = useState<ReviewStatus | 'all'>(initialFilter)
  ...
  async function fetchApplications(targetFilter: ReviewStatus | 'all', targetPage: number) {
    setLoading(true)
    ...
    if (targetFilter !== 'all') {
      query = query.eq('review_status', targetFilter)
    }
  ...
  function handleFilterChange(newFilter: ReviewStatus | 'all') {
    setFilter(newFilter)
    setPage(0)
    fetchApplications(newFilter, 0)
  }

  function handlePageChange(newPage: number) {
    setPage(newPage)
    fetchApplications(filter, newPage)
  }
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
            ...
            <textarea
              ...
              rows={3}
              placeholder="e.g. Portfolio doesn't meet our current bar — feel free to reapply after updating your samples."
                ...
                <div className="flex items-center gap-4 px-5 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {cp?.discipline && <span className="text-xs text-gray-400">· {cp.discipline}</span>}
                      {cp?.location && <span className="text-xs text-gray-400">· {cp.location}</span>}
                    </div>
                    <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                      ...
                      {cp?.availability && (
                        <p className="text-xs text-gray-400 capitalize">{cp.availability.replace(/_/g, ' ')}</p>
                      )}
